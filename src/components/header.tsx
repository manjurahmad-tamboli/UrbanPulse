...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-64 right-0 z-30 h-14 bg-[#0d1117]/90 backdrop-blur-xl border-b border-cyan-500/10 flex items-center justify-between px-5">
        ...
        {scenarioActive && (
          ...
          >
            <span className="text-xs text-cyan-400 font-medium">
              Step {currentScenarioStep + 1}/{scenarioSteps.length}:
            </span>
            ...
            <button
              ...
              className="ml-2 px-2 py-0.5 text-xs bg-cyan-500/20 text-cyan-400 rounded hover:bg-cyan-500/30 transition-colors flex items-center gap-1"
            >
              {currentScenarioStep < scenarioSteps.length - 1 ? 'Next' : 'Finish'}
              <ChevronRight className="w-3 h-3" />
            </button>
            <button
              onClick={resetScenario}
              className="px-1.5 py-0.5 text-xs text-gray-400 hover:text-white transition-colors"
            >
      ...
      <div className="flex items-center gap-2">
        ...
        {!scenarioActive && (
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-lg bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-400 border border-cyan-500/20 hover:border-cyan-500/40 transition-all cursor-pointer"
          >
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-lg border transition-all cursor-pointer',
            isSimulating
        ...
        >
          ...
          {isSimulating ? 'Simulation Live' : 'Start Simulation'}
        </button>
